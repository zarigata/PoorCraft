...
public class PlayerController {
    ...
    public Vector3f getEyePosition() {
        return new Vector3f(position).add(0.0f, EYE_HEIGHT, 0.0f);
    }
    ...
    public void update(World world, InputHandler input, Settings settings, Camera camera, float deltaTime) {
        ...
        int sneakKey = settings.controls.getKeybind("sneak", GLFW_KEY_LEFT_SHIFT);
        int sprintKey = settings.controls.getKeybind("sprint", GLFW_KEY_LEFT_CONTROL);
        ...
        } else if (input.isKeyPressed(sneakKey)) {
        ...
        boolean jumpRequested = input.isKeyPressed(jumpKey);
        ...
        moveAxis(world, moveX, Axis.X);
        moveAxis(world, moveY, Axis.Y);
        moveAxis(world, moveZ, Axis.Z);
